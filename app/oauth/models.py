...
class Server(models.Model):
    ...
    tls_client_certificate_bound_access_tokens = models.BooleanField(
        "Bind Access Tokens to TLS Client Certificate", default=False
    )
...
class Session(models.Model):
    ...
    client = models.ForeignKey(
        Client, on_delete=models.CASCADE, null=True, default=None, blank=True
    )
    ...
    code_challenge_method = models.TextField(
        max_length=5, choices=CODE_CHALLENGE_METHODS, blank=True
    )
    redirect_uri = models.URLField(null=True, default=None, blank=True)
    response_type = models.CharField(
        max_length=24, choices=ResponseType.choices(), null=True, default=None
    )
    state = models.CharField(max_length=1024, blank=True)
    access_token = models.CharField(
        max_length=AT_LENGTH, db_index=True, unique=True, null=True, blank=True
    )
    refresh_token = models.CharField(
        max_length=AT_LENGTH, db_index=True, unique=True, null=True, blank=True
    )
    authorization_code = models.CharField(
        max_length=CODE_LENGTH, db_index=True, unique=True, null=True, blank=True
    )
    tls_certificate = models.TextField(
        "TLS Certificate to bind the access token to", blank=True,
    )
    ...
    request_uri = models.CharField(
        max_length=1024, db_index=True, unique=True, null=True, blank=True
    )
    ...
    def create_from_request(cls, server, authenticated_client=None, kwargs={}):
        ...
        if "code_challenge" in kwargs and (
            not "code_challenge_method" in kwargs
            or kwargs["code_challenge_method"] != "S256"
        ):
            raise Exception(
                f"Missing or illegal code_challenge_method ('{kwargs.get('code_challenge_method', '')}')"
            )
    ...
    def pkce_verify(self, code_verifier):
        ...
        if not self.code_challenge == compare:
            raise Exception(
                f"Wrong PKCE verifier '{compare}' does not equal '{self.code_challenge}'"
            )
    ...
    def build_dependent_authorizations(self):
        ...
        for ad in self.authorization_details:
            for rule in settings.AUTHZ_DEPENDENCIES:
                if rule["type"] != ad["type"] or rule.get(
                    "locations", ""
                ) not in ad.get("location", ""):
                    continue
                for creation_rule in rule["depends_on"]:
                    ...
                    session.authorization_details = creation_rule.get(
    ...
    def __str__(self):
        myname = (
            f"{self.id}→{self.client.name if self.client else self.client_location}"
        )
        if self.depends_on:
