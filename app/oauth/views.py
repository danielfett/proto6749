...
class AccessTokenRequiredMixin:
    def dispatch(self, request, *args, **kwargs):
        ...
        session = Session.objects.get(access_token=access_token,)
...
class MetadataEndpoint(AuthzServerView):
    def get(self, request, *args, **kwargs):
        ...
        response["token_endpoint_auth_methods_supported"] = [
            method[0] for method in client_auth_methods_supported
        ]
        response["code_challenge_methods_supported"] = ["S256"]
        response[
            "tls_client_certificate_bound_access_tokens"
        ] = self.server.tls_client_certificate_bound_access_tokens
...
class PushedRequestEndpoint(ClientAuthRequiredMixin, AuthzServerView):
    def post(self, request, *args, **kwargs):
        session = Session.create_from_request(
            self.server, self.client_verified, request.POST
        )
...
class AuthorizationEndpoint(AuthzServerView):
    def get(self, request, *args, **kwargs):
        if "request_uri" in request.GET:
            ...
            session = Session.objects.get(request_uri=request.GET["request_uri"])
        else:
            session = Session.create_from_request(
                self.server, authenticated_client=None, kwargs=request.GET
            )
            session.save()
...
class TokenEndpoint(ClientAuthRequiredMixin, AuthzServerView):
    def post(self, request, *args, **kwargs):
        ...
        if self.server.tls_client_certificate_bound_access_tokens:
            try:
                session.tls_certificate = request.headers[
                    settings.TLS_CLIENT_CERTIFICATE_HEADER
                ]
            except KeyError:
                raise Exception("TLS Certificate not sent by client")
    ...
    def token_exchange(self, request):
        if (
            request.POST.get("subject_token_type")
            != "urn:ietf:params:oauth:token-type:access_token"
        ):
            raise Exception("Invalid or missing subject_token_type")

        subject_token = request.POST["subject_token"]
        master_session = Session.objects.get(access_token=subject_token)
        sessions = list(
            master_session.find_dependent_session(
                client=self.client_verified, location=request.POST.get("resource", None)
            )
        )

        if len(sessions) != 1:
            raise Exception(
                f"Expected to find exactly 1 session, but found {len(sessions)}."
            )
...
class DummyResourceEndpoint(AccessTokenRequiredMixin, AuthzServerView):
    def get(self, request, *args, **kwargs):
        ...
        for session in self.session_from_token_verified.find_dependent_session(
            None, None
        ):
